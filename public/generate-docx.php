 ...
 *
 * Usage:
 *   Open in browser: http://localhost/permissions/generate-docx.php
 *   Or run from CLI: php public/generate-docx.php
 */
...
// ── Read Markdown ──────────────────────────────────────────────────────
$md = file_get_contents(ROOT_PATH . '/docs/DOCUMENTATION.md');
if (!$md) {
...
$cell2->addText('Τεχνική Τεκμηρίωση v2.0', ['size' => 8, 'color' => '888888', 'italic' => true], ['alignment' => Jc::END, 'spaceAfter' => 0]);
...
// ── Save ───────────────────────────────────────────────────────────────
$outputPath = ROOT_PATH . '/docs/DOCUMENTATION.docx';
$writer = IOFactory::createWriter($phpWord, 'Word2007');
$writer->save($outputPath);

$msg = "DOCUMENTATION.docx δημιουργήθηκε επιτυχώς!";
if (php_sapi_name() === 'cli') {
    echo $msg . "\nPath: {$outputPath}\n";
} else {
    echo "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>Generate DOCX</title></head><body>";
    echo "<div style='font-family:Calibri,Arial,sans-serif;padding:60px;text-align:center;'>";
    echo "<div style='font-size:48px;color:#2c5f8a;margin-bottom:20px;'>&#10004;</div>";
    echo "<h2 style='color:#1a3c5e;'>{$msg}</h2>";
    echo "<p style='color:#666;'>Αρχείο: <code>{$outputPath}</code></p>";
    echo "<p style='margin-top:30px;'><a href='/permissions/docs/DOCUMENTATION.docx' ";
    echo "style='background:#2c5f8a;color:white;padding:12px 30px;text-decoration:none;border-radius:6px;font-size:16px;'>";
    echo "&#11015; Λήψη DOCUMENTATION.docx</a></p>";
    echo "</div></body></html>";
}
